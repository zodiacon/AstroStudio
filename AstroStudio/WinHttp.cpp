...
bool HttpRequest::http(const std::wstring& verb, const std::wstring& user_agent, const std::wstring& domain,
	...
	DWORD dwAccessType = ::IsWindows8OrGreater() ?
	...
	while (!bDone) {
		...
		if (hRequest) {
			if (requestHeader.empty()) {
				...
				bResults = WinHttpSendRequest(hRequest,
					requestHeader.c_str(), (DWORD)requestHeader.size(),
					(LPVOID)body.data(), (DWORD)body.size(),
					(DWORD)body.size(), 0);
			}
			if (!bResults) {
...
DWORD HttpRequest::ChooseAuthScheme(DWORD dwSupportedSchemes) {
...
std::unordered_map<std::wstring, std::wstring>& HttpResponse::GetHeaderDictionary() {
	if (!dict.empty())
		return dict;
