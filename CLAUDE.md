...
- **`AstroStudio/`** — The WTL application.
  ...
  - `TimeControls` / `LocationControls` — the date (day/month combos + year edit, so pre-1752 and Julian-calendar dates work), time, time zone and lat/lon controls, shared by `ChartDetailsView` and `NewChartDlg` through common control IDs (`IDC_DAY`, `IDC_LATDEG`, ...). Owners keep the message handling and call into these. Dates before 15 Oct 1582 are Julian; the year range is the built-in ephemeris' (-3000..3000, 0 = 1 BC).
